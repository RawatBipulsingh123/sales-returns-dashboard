...
data uploaded as an Excel (.xlsx) file.
"""
...
def generate_excel_export(aggregation_data: dict) -> bytes:
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        main_title_format = workbook.add_format({"bold": True, "font_size": 15, "font_color": "#1F4E78"})
        section_title_format = workbook.add_format({"bold": True, "font_size": 12, "font_color": "#1F4E78", "bottom": 1})
        ...
        red_percent_format = workbook.add_format({"num_format": "0.00%", "font_color": "#C00000", "bold": True, "border": 1})
...
try:
    ...
    st.error(f"❌ Could not read the uploaded file. It may be corrupted or not a valid Excel file.\n\nDetails: {e}")
    st.stop()
...
if len(set(mapping.values())) < len(mapping):
    st.sidebar.warning("⚠️ Two or more fields are mapped to the same source column.")
st.sidebar.markdown("---")
...
if df_processed.empty:
    st.warning("After validation, no rows contain a usable Date. Please check your column mapping.")
    st.stop()

# --------------------------------------------------------------------------
# Main Layout & Tab Setup (Fixed Collisions)
# --------------------------------------------------------------------------
...
tab_labels = [str(y) for y in years] + ["Overall", "🔍 Item Search", "🔨 Custom Visualizations", "📦 Stock vs Sales", "📈 Day Trends", "📊 Executive KPI", "💸 Salesman ROI", "🚫 Toxic Product Radar"]
...
with tabs[len(years) + 1]:
    ...
    with f_row1_1:
        if pd.notnull(min_dt) and pd.notnull(max_dt):
            sel_dates = st.date_input("📅 Global Date Filter", value=[], min_value=min_dt.date(), max_value=max_dt.date(), key="top_date")
        else:
            sel_dates = []
        ...
        if stock_file is not None:
            try:
                ...
                if stock_file.name.endswith('.csv'):
                    df_stock_search = pd.read_csv(stock_file, skiprows=6, low_memory=False)
                    if "StoreName" not in df_stock_search.columns and "StockInHand" not in df_stock_search.columns:
                        stock_file.seek(0)
                        df_stock_search = pd.read_csv(stock_file, low_memory=False)
                else:
                    df_stock_search = pd.read_excel(stock_file, skiprows=6)
                    if "StoreName" not in df_stock_search.columns and "StockInHand" not in df_stock_search.columns:
                        stock_file.seek(0)
                        df_stock_search = pd.read_excel(stock_file)
                ...
                if all(col in df_stock_search.columns for col in ["EANCode", "StoreName", "StockInHand"]):
                    ...
                    if not stock_filtered.empty:
                        ...
                        st.dataframe(stock_agg.sort_values("StockInHand", ascending=False).reset_index(drop=True), use_container_width=True)
                    else:
                        st.warning("No inventory found.")
                else:
                    st.warning("⚠️ Stock file missing critical columns.")
            except Exception as e:
                st.error(f"❌ Error: {e}")
        else:
            st.info("Upload Stock file to view.")
...
with tabs[len(years) + 2]:
    ...
    with f1:
        year_opts = ["All"] + sorted(viz_df["Year"].dropna().astype(str).unique().tolist()) if "Year" in viz_df.columns else ["All"]
        sel_year = st.selectbox("Filter Year", year_opts, key="g_year")
        if sel_year != "All": viz_df = viz_df[viz_df["Year"].astype(str) == sel_year]
    with f2:
        prod_opts = ["All"] + sorted(viz_df["Product"].dropna().unique().tolist()) if "Product" in viz_df.columns else ["All"]
        sel_prod = st.selectbox("Filter Product", prod_opts, key="g_prod")
        if sel_prod != "All": viz_df = viz_df[viz_df["Product"] == sel_prod]
    with f3:
        store_opts = ["All"] + sorted(viz_df["Store Name"].dropna().unique().tolist())
        sel_store = st.selectbox("Filter Store", store_opts, key="g_store")
        if sel_store != "All": viz_df = viz_df[viz_df["Store Name"] == sel_store]
    with f4:
        color_opts = ["All"] + sorted(viz_df["Color"].dropna().unique().tolist())
        sel_color = st.selectbox("Filter Color", color_opts, key="g_color")
        if sel_color != "All": viz_df = viz_df[viz_df["Color"] == sel_color]
        
    st.divider()
    def render_chart(container, chart_id):
        with container:
            c_a, c_b = st.columns(2)
            with c_a: x_axis = st.selectbox(f"X-Axis {chart_id}", viz_df.columns.tolist(), key=f"x_{chart_id}")
            with c_b: y_axis = st.selectbox(f"Y-Axis {chart_id}", viz_df.columns.tolist(), index=min(1, len(viz_df.columns)-1), key=f"y_{chart_id}")
            c_c, c_d = st.columns(2)
            with c_c: chart_type = st.selectbox(f"Type {chart_id}", ["Bar", "Line", "Pie"], key=f"type_{chart_id}")
            with c_d: top_n = st.number_input(f"Top 'N' {chart_id}", min_value=1, max_value=50, value=10, key=f"top_{chart_id}")

            if x_axis != y_axis and y_axis in viz_df.select_dtypes(include=[np.number]).columns:
                try:
                    plot_df = viz_df[viz_df[x_axis].astype(str) != "TOTAL"].groupby(x_axis, as_index=False)[y_axis].sum()
                    if chart_type in ("Bar", "Pie"): plot_df = plot_df.sort_values(y_axis, ascending=False).head(top_n)
                    
                    if chart_type == "Bar": fig = px.bar(plot_df, x=x_axis, y=y_axis, text_auto=True)
                    elif chart_type == "Line": fig = px.line(plot_df.sort_values(x_axis), x=x_axis, y=y_axis, markers=True)
                    else: fig = px.pie(plot_df, names=x_axis, values=y_axis)
...
# 5. Stock vs Sales Analysis Tab
with tabs[len(years) + 3]:
    st.markdown("### ⚖️ Comprehensive Stock vs Sales Analysis")
    if stock_file is None:
        st.info("👈 Upload Stock Data (CSV/Excel) in sidebar to unlock.")
    else:
        try:
            stock_file.seek(0)
            if stock_file.name.endswith('.csv'):
            ...
            col_map = {"Eancode": "EANCode", "Productname": "ProductName", "Stock Qty. ": "StockInHand", "Stock Qty.": "StockInHand", "Storename": "StoreName", "Color": "ColorName", "Size": "SizeName"}
            df_stock_raw.rename(columns=col_map, inplace=True)
            
            if not all(c in df_stock_raw.columns for c in ["EANCode", "StoreName", "StockInHand"]):
                st.error("⚠️ Missing critical columns in Stock file.")
            else:
                df_stock = df_stock_raw.rename(columns={"ColorName": "Color", "SizeName": "Size", "StoreName": "Store Name", "ProductName": "Product"}).copy()
                ...
                av_years = ["Overall"] + sorted(df_processed["Year"].dropna().astype(int).unique().tolist())
                
                with col1: selected_year = st.selectbox("📅 Select Year", av_years)
                with col2:
                    if search_mode == "Product Name":
                        selected_items = st.multiselect("👕 Select Products (Leave blank for All)", sorted(df_processed["Product"].dropna().unique().tolist()))
                    else:
                        selected_items = st.multiselect("🏷️ Select EAN Codes", sorted(df_processed["EANCode"].unique().tolist()))
                
                col3, col4 = st.columns([3, 1])
                with col3:
                    selected_stores = st.multiselect("🏬 Select Stores", sorted(df_processed["Store Name"].unique().tolist()))
                with col4:
                ...
                filtered_sales = df_processed.copy()
                filtered_stock = df_stock.copy()
                
                if selected_items:
                    if search_mode == "Product Name":
                        filtered_sales = filtered_sales[filtered_sales["Product"].isin(selected_items)]
                        filtered_stock = filtered_stock[filtered_stock["Product"].astype(str).str.upper().isin([x.upper() for x in selected_items])]
                    else:
                        filtered_sales = filtered_sales[filtered_sales["EANCode"].isin(selected_items)]
                        filtered_stock = filtered_stock[filtered_stock["EANCode"].astype(str).str.upper().isin([x.upper() for x in selected_items])]
                ...
                def generate_table(sales_df, stock_df, group_col):
                    if group_col not in sales_df.columns or group_col not in stock_df.columns: return None
                    s_agg = sales_df.groupby([group_col], as_index=False)["Quantity"].sum().rename(columns={"Quantity": "Sales Qty"})
                    ret_agg = sales_df[sales_df["Quantity"] < 0].groupby([group_col], as_index=False)["Quantity"].sum().rename(columns={"Quantity": "Return Qty"})
                    k_agg = stock_df.groupby([group_col], as_index=False)["StockInHand"].sum()
                    
                    merged = pd.merge(pd.merge(s_agg, k_agg, on=[group_col], how="outer"), ret_agg, on=[group_col], how="left").fillna(0)
                    for c in ["Sales Qty", "Return Qty", "StockInHand"]: merged[c] = merged[c].astype(int)
                    merged["Send Qty"] = merged["Sales Qty"] + merged["StockInHand"]
                    merged["Percentage"] = ((merged["Sales Qty"] / merged["Send Qty"].replace(0, 1)) * 100).round(1).astype(str) + "%"
                    
                    display_cols = [group_col, "Sales Qty"]
                    if show_returns: display_cols.append("Return Qty")
                    if show_send_qty: display_cols.extend(["Send Qty", "StockInHand", "Percentage"])
                    else: display_cols.append("StockInHand")
                    return merged[display_cols].sort_values(by="Sales Qty", ascending=False).reset_index(drop=True)

                items_to_display = selected_items if selected_items else ["All Products"]
                for item in items_to_display:
                    st.markdown(f"### 📦 Analytics for: **{item}**")
                    item_sales_ui = filtered_sales if item == "All Products" else (filtered_sales[filtered_sales["Product"]==item] if search_mode=="Product Name" else filtered_sales[filtered_sales["EANCode"]==item])
                    item_stock_ui = filtered_stock if item == "All Products" else (filtered_stock[filtered_stock["Product"].str.upper()==item.upper()] if search_mode=="Product Name" else filtered_stock[filtered_stock["EANCode"].str.upper()==item.upper()])
                    ...
                    c1, c2 = st.columns(2)
                    with c1:
                        st.markdown("#### 🎨 Color-wise")
                        df_color = generate_table(item_sales_ui, item_stock_ui, "Color")
                        if df_color is not None: st.dataframe(df_color, use_container_width=True)
                    with c2:
                        st.markdown("#### 📏 Size-wise")
...
# 6. Day-wise Trend Analysis Tab
with tabs[len(years) + 4]:
    st.markdown("### 📅 Day-wise Trend Analysis")
    df_trend = df_processed.dropna(subset=["Date"]).copy()
    ...
    with col2:
        ...
        selected_item_trend = st.selectbox("Product", ["All"] + sorted(df_trend["Product"].unique().tolist()), label_visibility="collapsed")
    with col3:
        st.markdown("##### 🏬 Store Slicer")
        selected_stores_trend = st.multiselect("Store", sorted(df_trend["Store Name"].unique().tolist()), label_visibility="collapsed")
        
    if selected_item_trend != "All": df_trend = df_trend[df_trend["Product"] == selected_item_trend]
    if selected_stores_trend: df_trend = df_trend[df_trend["Store Name"].isin(selected_stores_trend)]
        
    st.divider()
    if not df_trend.empty:
        day_order = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]
        plot_df = df_trend.groupby("DayOfWeek")["Quantity"].sum().reindex(day_order).fillna(0).reset_index()
        fig = px.bar(plot_df, x="DayOfWeek", y="Quantity", text="Quantity", color_discrete_sequence=["#1F4E78"])
...
with tabs[len(years) + 5]:
    ...
    with sl1:
        ex_year = st.multiselect("Filter Year", sorted(exec_df["Year"].dropna().astype(int).unique().tolist()))
        if ex_year: exec_df = exec_df[exec_df["Year"].isin(ex_year)]
    with sl2:
        ex_store = st.multiselect("Filter Store", sorted(exec_df["Store Name"].unique().tolist()))
        if ex_store: exec_df = exec_df[exec_df["Store Name"].isin(ex_store)]
    with sl3:
        ex_prod = st.multiselect("Filter Product", sorted(exec_df["Product"].unique().tolist()))
        if ex_prod: exec_df = exec_df[exec_df["Product"].isin(ex_prod)]
    with sl4:
        ex_color = st.multiselect("Filter Color", sorted(exec_df["Color"].unique().tolist()))
        if ex_color: exec_df = exec_df[exec_df["Color"].isin(ex_color)]
        
    st.divider()
    if not exec_df.empty:
        g = exec_df.loc[exec_df["Quantity"]>0, "Quantity"].sum()
        r = exec_df.loc[exec_df["Quantity"]<0, "Quantity"].sum()
        c1, c2, c3 = st.columns(3)
        c1.metric("Gross Units", f"{g:,.0f}")
        c2.metric("Returned Units", f"{r:,.0f}")
        c3.metric("Net Units", f"{g+r:,.0f}")
        st.divider()
...
with tabs[len(years) + 6]: 
    ...
    if missing:
        ...
        st.markdown("##### 🌍 Global Company Overview")
        g1, g2, g3, g4 = st.columns(4)
        g_sales = roi_df_base['Amount'].sum()
        g_margin = roi_df_base['Gross Margin'].sum()
        g1.metric("Total Revenue", f"₹{g_sales:,.0f}")
        g2.metric("Total Cost", f"₹{roi_df_base['Total Cost'].sum():,.0f}")
        g3.metric("Gross Margin", f"₹{g_margin:,.0f}")
        g4.metric("Discount Leaked", f"₹{roi_df_base['Discount Given'].sum():,.0f}")
        st.divider()

        st.markdown("##### ⚙️ Advanced Filters")
        f1, f2 = st.columns(2)
        roi_df_filtered = roi_df_base.copy() 
        ...
        with f1:
            ...
            if store_col:
                sel_stores = st.multiselect("Filter by Store", sorted(roi_df_filtered[store_col].dropna().astype(str).unique().tolist()), key="roi_store")
                if sel_stores: roi_df_filtered = roi_df_filtered[roi_df_filtered[store_col].isin(sel_stores)]
                
        with f2:
            sel_salesman = st.multiselect("Filter by Salesman", sorted(roi_df_filtered['Salesman'].dropna().astype(str).unique().tolist()), key="roi_salesman")
            if sel_salesman: roi_df_filtered = roi_df_filtered[roi_df_filtered['Salesman'].isin(sel_salesman)]
        ...
        if roi_df_filtered.empty:
            ...
            k1.metric("Filtered Revenue", f"₹{roi_df_filtered['Amount'].sum():,.0f}")
            k2.metric("Filtered Cost", f"₹{roi_df_filtered['Total Cost'].sum():,.0f}")
            k3.metric("Filtered Margin", f"₹{roi_df_filtered['Gross Margin'].sum():,.0f}")
            k4.metric("Filtered Discount", f"₹{roi_df_filtered['Discount Given'].sum():,.0f}")
            
            st.divider()
            st.markdown("##### 🏆 Salesman Leaderboard")
            salesman_agg = roi_df_filtered.groupby('Salesman', as_index=False).agg(
                Units_Sold=('Quantity', 'sum'), Total_Revenue=('Amount', 'sum'),
                Gross_Margin=('Gross Margin', 'sum'), Discount_Given=('Discount Given', 'sum')
...
with tabs[len(years) + 7]:
    ...
    if all(col in tox_df.columns for col in ['Product', 'Quantity', 'Amount']):
        ...
        returns_data = tox_df[tox_df['Quantity'] < 0].groupby('Product', as_index=False).agg(Units_Returned=('Quantity', 'sum'), Revenue_Lost=('Amount', 'sum'))
        ...
        toxic_agg['Return_Rate_%'] = toxic_agg.apply(lambda r: (r['Units_Returned'] / r['Units_Sold'] * 100) if r['Units_Sold'] > 0 else 0, axis=1)
...
try:
    ...
    st.sidebar.download_button("Download Dashboard (.xlsx)", excel_bytes, "Sales_Returns_Dashboard.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
except Exception as e:
